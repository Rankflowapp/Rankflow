...
export default async function CoachPage({ params }) {
  ...
  // Alertes agent/map problématiques
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold mt-2">{account.name}'s Coach</h1>
        <p className="text-slate-400 text-sm">Personalized advice for your session</p>
      </div>
        ...
        <div className="space-y-2">
            ...
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border-l-4 ${
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-white">
                  {streakType === "Loss" ? "Loss streak in progress" : "Win streak in progress"}
                </p>
                <p className="text-xs text-slate-400">
                  {streakType === "Loss"
                    ? "Take a break or switch game mode, tilt is showing"
                    : "Keep this momentum going, you're in the zone"}
                </p>
              </div>
          ...
          {problematicAgent && (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border-l-4 bg-amber-500/5 border-l-amber-500 border-y border-r border-amber-500/20">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-white">
                  Struggling agent: {problematicAgent.agent}
                </p>
                <p className="text-xs text-slate-400">
                  Keep this pick for customs until you've practiced it more
                </p>
              </div>
          ...
          {problematicMap && (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border-l-4 bg-orange-500/5 border-l-orange-500 border-y border-r border-orange-500/20">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-white">
                  Difficult map: {problematicMap.map}
                </p>
                <p className="text-xs text-slate-400">
                  Practice this map in unrated to improve your setups
                </p>
              </div>
      ...
      <div className="bg-gradient-to-br from-indigo-500/10 via-slate-900 to-slate-900 border border-indigo-500/30 rounded-3xl p-6">
        <p className="text-xs text-indigo-300 uppercase tracking-wider font-semibold mb-4">🎯 Your Session Plan</p>

        <div className="mb-6">
          <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-2">Session goal</p>
          {(() => {
            ...
            if (netScore < -2) {
              objective = "Stop the bleeding, aim for +10 RR minimum"
              objectiveColor = "text-rose-400"
            } else if (netScore > 2) {
              objective = "Confirm the climb, aim for +20 RR this session"
              objectiveColor = "text-emerald-400"
            } else {
              objective = "Breakthrough, aim for +25 RR to push forward"
        ...
        <div className="space-y-3 mb-6">
          <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Your action plan</p>
          ...
          <div className="flex gap-3 items-start bg-slate-800/50 border border-slate-800 rounded-2xl p-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Maps to prioritize</p>
              {bestMap ? (
                <p className="text-sm text-slate-400 mt-1">
                  Play <span className="text-emerald-400 font-semibold">{bestMap.map}</span> whenever it comes up — {bestMap.wr}% WR ({bestMap.wins}W/{bestMap.losses}L)
                </p>
              ) : (
                <p className="text-sm text-slate-500 mt-1">Not enough data on your maps yet</p>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3 items-start bg-slate-800/50 border border-slate-800 rounded-2xl p-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Recommended pick</p>
              {bestAgent ? (
                <p className="text-sm text-slate-400 mt-1">
                  Pick <span className="text-indigo-400 font-semibold">{bestAgent.agent}</span> — you're at {bestAgent.wr}% WR ({bestAgent.wins}W/{bestAgent.losses}L)
                </p>
              ) : (
                <p className="text-sm text-slate-500 mt-1">Not enough data on your agents yet</p>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3 items-start bg-slate-800/50 border border-slate-800 rounded-2xl p-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Watch out for</p>
              {worstMap || worstAgent ? (
                <p className="text-sm text-slate-400 mt-1">
                  {worstMap && <>Practice <span className="text-amber-400 font-semibold">{worstMap.map}</span> in unrated ({worstMap.wr}% WR)</>}
                  {worstMap && worstAgent && " · "}
                  {worstAgent && <>Keep <span className="text-amber-400 font-semibold">{worstAgent.agent}</span> for customs ({worstAgent.wr}% WR)</>}
                </p>
              ) : (
                <p className="text-sm text-slate-500 mt-1">No major watch points detected</p>
              )}
            </div>
          </div>
        </div>
        ...
        {bestMap && bestAgent && (
          <div className="bg-slate-800/50 border border-indigo-500/30 rounded-2xl p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-2">📊 Prediction</p>
            {(() => {
              const avgWr = Math.round((bestMap.wr + bestAgent.wr) / 2)
              return (
                <p className="text-sm text-slate-300 leading-relaxed">
                  Following this plan, you have <span className="text-2xl font-bold text-indigo-300">{avgWr}%</span> chance of reaching your session goal.
                </p>
              )
            })()}
          </div>
      ...
      <div className="bg-gradient-to-br from-violet-500/10 via-slate-900 to-slate-900 border border-violet-500/30 rounded-3xl p-6">
        <div className="flex items-center gap-2 mb-4">
          <p className="text-xs text-violet-300 uppercase tracking-wider font-semibold">🎯 Tips for YOU</p>
        ...
        {personalizedTips.length > 0 ? (
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/30 rounded-2xl p-5 text-center">
            ...
            <p className="text-sm font-semibold text-emerald-400 mb-1">No critical weaknesses detected</p>
            <p className="text-sm text-slate-400">
              You're solid on all fundamentals. Now focus on optimizing your best agent and push your trades even higher.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        {bestMap && (
          <div className="bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/30 rounded-3xl p-6">
            <p className="text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-2">🗺️ Your best map</p>
            <p className="text-3xl font-bold text-white">{bestMap.map}</p>
            <p className="text-slate-400 text-sm mt-1">{bestMap.wins}W · {bestMap.losses}L · {bestMap.wr}% winrate</p>
            <p className="text-sm text-slate-300 mt-3">Prioritize this map during your competitive sessions.</p>
          </div>
        )}

        {bestAgent && (
          <div className="bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/30 rounded-3xl p-6">
            <p className="text-indigo-400 text-xs font-semibold uppercase tracking-wider mb-2">🎭 Your best agent</p>
            <p className="text-3xl font-bold text-white">{bestAgent.agent}</p>
            <p className="text-slate-400 text-sm mt-1">{bestAgent.wins}W · {bestAgent.losses}L · {bestAgent.wr}% winrate</p>
            <p className="text-sm text-slate-300 mt-3">This is your main pick, keep mastering it.</p>
          </div>
        )}
